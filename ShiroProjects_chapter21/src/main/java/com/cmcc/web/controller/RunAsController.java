...
import com.cmcc.service.UserRunAsService;
...
public class RunAsController {
    ...
    public String runasList(@CurrentUser User loginUser, Model model) {
        model.addAttribute("fromUserIds", userRunAsService.findFromUserIds(loginUser.getId()));     //获取谁给我授过权
        model.addAttribute("toUserIds", userRunAsService.findToUserIds(loginUser.getId()));         //获取我给谁授过权
        ...
        if(subject.isRunAs()) {
            String previousUsername = (String) subject.getPrincipals().getPrimaryPrincipal();
