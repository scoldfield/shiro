...
public class OnlineSessionFilter extends AccessControlFilter{
    ...
    protected boolean isAccessAllowed(ServletRequest request,
        ...
        if(subject == null || subject.getSession(false) == null) {  //subject.getSession(false)表示在调用getSession()方法时，若session不存在，那么不为该subject创建session；若设为true，那么为该subject创建session
            return true;
        }
